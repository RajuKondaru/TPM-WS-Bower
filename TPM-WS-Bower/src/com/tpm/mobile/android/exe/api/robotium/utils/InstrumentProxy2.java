...
public class InstrumentProxy2 {
	private static final Logger log = Logger.getLogger(InstrumentProxy2.class);
	...
	public JSONObject start(JSONObject obj) throws IOException{
		 String instruments = null;
			...
			String lines;
			String DEVICE_VIDEO_PATH = "/sdcard/"+test.get("name").toString()+".mp4";//
				...
				while((lines=r.readLine()) != null ){
					if(!lines.isEmpty()){
						if(lines.contains("100%")){
							//log.info(lines);
							String dalateCommand= "adb -s "+deviceId+" shell rm -f /sdcard/"+test.get("name").toString()+".mp4";
			...
			resultJson.put("vedioPath", FileUtils.getUserDirectory()+"/AppData/Local/Temp/"+test.get("name").toString()+".mp4");
