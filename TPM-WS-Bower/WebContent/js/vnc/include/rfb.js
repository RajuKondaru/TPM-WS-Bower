...
init_msg = function() {
    ...
    case 'ServerInitialisation' :
        ...
        //fb_width = fb_width*33/100;
        //fb_height = fb_height*33/100;
        /* PIXEL_FORMAT */
        bpp            = ws.rQshift8();
        depth          = ws.rQshift8();
        ...
        console.log("Screen: " + fb_width + "x" + fb_height + 
                ", bpp: " + bpp + ", depth: " + depth +
        ...
        console.log(">> ServerInitialisation :: "+"w: "+fb_width+"h: "+fb_height);
...
framebufferUpdate = function() {
    ...
    while (FBU.rects > 0) {
        ...
        if (FBU.bytes === 0) {
            ...
            //console.log("framebufferUpdate ::"+ hdr);
            /*framebufferUpdate ::0,3,0,3,0,8,0,7,255,255,255,17*/
            FBU.x      = (hdr[0] << 8) + hdr[1];
            FBU.y      = (hdr[2] << 8) + hdr[3];
...
encHandlers.COPYRECT = function display_copy_rect() {
    ...
    display.renderQ_push({
            ...
            'width': FBU.width,
            'height': FBU.height});
    console.log('From encHandlers.COPYRECT << type : copy , old_x :'+ ws.rQshift16()+'old_y :'+ ws.rQshift16()+ 'x :'+ FBU.x+ 'y :'+ FBU.y+ 'width :'+ FBU.width+'height :'+ FBU.height);
    FBU.rects -= 1;
    FBU.bytes = 0;
    return true;
};
...
function display_tight(isTightPNG) {
	...
	//console.log("from display_tight");
    if (fb_depth === 1) {
...
extract_data_uri = function(arr) {
	//console.log("from extract_data_uri");
    //var i, stra = [];
...
encHandlers.TIGHT = function () { 
	
	return display_tight(false); };
...
encHandlers.DesktopSize = function set_desktopsize() {
    ...
    console.log(">> set_desktopsize :: "+"w: "+fb_width+"h: "+fb_height);
    display.resize(fb_width, fb_height);
...
encHandlers.Cursor = function set_cursor() {
    ...
    FBU.bytes = 0;
    FBU.rects -= 1;
    console.log("from set_cursor, x: " + x + ", y: " + y + ", w: " + w + ", h: " + h + ", pixelslength: " + pixelslength + ", masklength: " + masklength);
    Util.Debug("<< set_cursor");
    return true;
};
...
that.sendPassword = function(passwd) {
	console.log(">> sendPassword"+ passwd);
    rfb_password = passwd;
